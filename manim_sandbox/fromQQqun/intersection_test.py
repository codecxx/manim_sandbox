...
class Intersection_2circle(Irregular_shape):
    ...
    def __init__(self, Mcircle_1, Mcircle_2, **kwargs):
...
class Intersection_n_circle(VMobject):
    ...
    def __init__(self, *Mcircle, **kwargs):
        def p_in_circle(p, circle_o, circle_r):
            return (sum((p - circle_o) ** 2) <= circle_r ** 2)
...
class Text_test(Scene):

    def construct(self):

        t = TextMobject('first line\\\\second line')
        self.play(Write(t))
        self.wait(2)
...
class Four_circles(Scene):
    ...
    def construct(self):
        ...
        intersection_1234 = Intersection_n_circle(circle_1, circle_2, circle_3, circle_4, color=YELLOW)
