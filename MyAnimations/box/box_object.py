...
class Box(VMobject):
    ...
    def __init__(self):
        ...
        tapaI = VMobject().set_points_as_corners(
            [self.points[0], self.points[0] + RIGHT * self.width * self.size_lid / 2]).set_stroke(width=self.lid_width)
        tapaD = VMobject().set_points_as_corners(
            [self.points[-1], self.points[-1] + LEFT * self.width * self.size_lid / 2]).set_stroke(width=self.lid_width)
...
class NoteBox(ImageMobject):
    def __init__(self):
        ImageMobject.__init__(self, filename_or_array=r'''note_box.png''')
